...
{
    SharedLinkConfigurationPtr sharedConfig;

    // 在已有的配置列表中查找传入的config是否已被管理
    for (int i = 0; i < _rgLinkConfigs.count(); i++) {
        if (_rgLinkConfigs[i].get() == config) {
            sharedConfig = _rgLinkConfigs[i];
            break;
        }
    }

    // 如果未找到，将config添加到配置列表中
    if (!sharedConfig) {
        sharedConfig = addConfiguration(config);
    }

    // 委托给重载版本，根据配置类型创建对应的链路并连接
