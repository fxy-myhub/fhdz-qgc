...
{
    if (_port && _port->isOpen()) {                     //  确保串口已打开
        qint64 byteCount = _port->bytesAvailable();     //  查询缓冲区有多少字节可读
        if (byteCount) {                                //  有数据才处理
            QByteArray buffer;
            buffer.resize(byteCount);                   // 分配 buffer 数组
            _port->read(buffer.data(), buffer.size());  // 从串口读取原始字节
            emit bytesReceived(this, buffer);           // 发射信号，把数据传给上层
        }
    } else {
        // Error occurred
        qWarning() << "Serial port not readable";       // 串口异常，报告错误
