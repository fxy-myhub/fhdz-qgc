...
Item {
    ...
    // 仪表盘下方遥测信息容器（重点信息栏配置，一个信息对应一个黑色透明框）
    DataDisplayBox {
        id:                         dataDisplayBox
        anchors.rightMargin:        0
        anchors.topMargin:          0
        ...
        // 绑定重点信息栏配置的选中状态
        engineChecked:              _root.parent.configMenuInstance.engineChecked
        gpsChecked:                 _root.parent.configMenuInstance.gpsChecked
        attitudeChecked:            _root.parent.configMenuInstance.attitudeChecked
        voltageChecked:             _root.parent.configMenuInstance.voltageChecked
        fuelChecked:                _root.parent.configMenuInstance.fuelChecked
        airspeedChecked:            _root.parent.configMenuInstance.airspeedChecked
        modeChecked:                _root.parent.configMenuInstance.modeChecked

        // 传入当前活动飞机，用于显示飞控姿态数据
        activeVehicle:              _root._activeVehicle

        // 传入重点信息栏配置弹窗共享实例，供容器内右键"配置"按钮打开
        configMenuInstance:         _root.parent.configMenuInstance
    }


    PhotoVideoControl {
        id:                     photoVideoControl
        visible:                false   // 隐藏拍照/录像按钮，避免误触触发相机命令警告
        anchors.margins:        _toolsMargin
        anchors.right:          parent.right
        width:                  _rightPanelWidth
    ...
    VehicleWarnings {
        ...
        visible:            false   // 隐藏 pre-arm 检查失败提示（如 Preflight Fail：High Accelerometer）
    }
    //地图比例尺
    // MapScale {
    //     id:                 mapScale
