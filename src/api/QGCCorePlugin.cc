...
QVariantList& QGCCorePlugin::analyzePages()
{
    if (!_p->analyzeList.count()) {
        ...
        // _p->analyzeList.append(QVariant::fromValue(new QmlComponentInfo(tr("SimpleTest"),     QUrl::fromUserInput("qrc:/qml/SimpleTest.qml"),      QUrl::fromUserInput("qrc:/qmlimages/MavlinkConsoleIcon"))));//新加
