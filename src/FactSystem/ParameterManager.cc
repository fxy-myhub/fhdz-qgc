...
/*
* 静态哈希表
* @brief _mavlinkCompIdHash MAVLink component ID hash table 
* @details This hash table is used to map MAVLink component IDs to their names. 
* @note This hash table is used to map MAVLink component IDs to their names.        
*/ 
...
void ParameterManager::_updateProgressBar(void)
{
    ...
    // Update readParamCount based on index-based parameter loading progress
    int newReadParamCount = _totalParamCount - waitingReadParamIndexCount;
    if (newReadParamCount < 0) {
        newReadParamCount = 0;
    }
    if (newReadParamCount != _readParamCount) {
        _readParamCount = newReadParamCount;
        emit readParamCountChanged(_readParamCount);
    }
...
void ParameterManager::refreshAllParameters(uint8_t componentId)
{
    ...
    // Reset readParamCount since we are re-reading all parameters
    if (_readParamCount != 0) {
        _readParamCount = 0;
        emit readParamCountChanged(_readParamCount);
    }
...
void ParameterManager::cancelReadAllParameters(void)
{
    qCDebug(ParameterManagerLog) << _logVehiclePrefix(-1) << "Cancel parameter read";
