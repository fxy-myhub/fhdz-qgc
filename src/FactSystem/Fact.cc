...
{
    if (_metaData) {    // 检查元数据是否存在
        QVariant    typedValue;     // 转换后的值
        QString     errorString;    // 错误字符串
        
        // 转换并验证原始值
        if (_metaData->convertAndValidateRaw(value, true /* convertOnly */, typedValue, errorString)) { // 转换并验证原始值
            if (typedValue != _rawValue) { // 检查转换后的值是否与当前值不同
                _rawValue.setValue(typedValue);
                _sendValueChangedSignal(cookedValue());
                //-- Must be in this order
                emit _containerRawValueChanged(rawValue());// 发送原始值变化信号
                emit rawValueChanged(_rawValue);     // 发送原始值变化信号
            }
        }
    } else {
        qWarning() << kMissingMetadata << name();
