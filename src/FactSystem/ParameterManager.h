...
public:
    ...
    Q_PROPERTY(int      readParamCount      READ readParamCount     NOTIFY readParamCountChanged)       ///< Number of parameters read so far
    ...
    int readParamCount      (void) const { return _readParamCount; }
    ...
    /// Cancel the current parameter read operation
    Q_INVOKABLE void cancelReadAllParameters(void);
...
signals:
    ...
    void readParamCountChanged      (int readParamCount);
...
private:
    ...
    int _readParamCount = 0;                    ///< Number of parameters read so far
