...
int main(int argc, char *argv[])
{
    ...
    /*防止多开   通过 RunGuard（基于共享内存实现的进程锁）检查是否已有 QGC 实例在运行。如果已有实例，
    弹窗提示用户并退出。避免多个 QGC 同时连接同一个飞控造成冲突*/
    ...
    // install the message handler 安装消息处理函数
...
#ifdef Q_OS_WIN
    QApplication::setWindowIcon(QIcon(":/res/img_add/ico/fenghua.png"));
#endif /* Q_OS_LINUX */
