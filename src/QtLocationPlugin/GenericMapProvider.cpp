...
// 高德地图 - 街道图
static const QString GaoDeStreetMapUrl = QStringLiteral("https://webrd0%1.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=7&x=%2&y=%3&z=%4");

QString GaoDeStreetMapProvider::_getURL(const int x, const int y, const int zoom, QNetworkAccessManager* networkManager) {
    Q_UNUSED(networkManager)
    return GaoDeStreetMapUrl.arg(_getServerNum(x, y, 4) + 1).arg(x).arg(y).arg(zoom);
}

// 高德地图 - 卫星图
static const QString GaoDeSatelliteMapUrl = QStringLiteral("https://webst0%1.is.autonavi.com/appmaptile?style=6&x=%2&y=%3&z=%4");

QString GaoDeSatelliteMapProvider::_getURL(const int x, const int y, const int zoom, QNetworkAccessManager* networkManager) {
    Q_UNUSED(networkManager)
    return GaoDeSatelliteMapUrl.arg(_getServerNum(x, y, 4) + 1).arg(x).arg(y).arg(zoom);
}

// 高德地图 - 路网标注（叠加在卫星图上形成混合地图）
static const QString GaoDeRoadMapUrl = QStringLiteral("https://webst0%1.is.autonavi.com/appmaptile?style=8&x=%2&y=%3&z=%4");

QString GaoDeRoadMapProvider::_getURL(const int x, const int y, const int zoom, QNetworkAccessManager* networkManager) {
    Q_UNUSED(networkManager)
    return GaoDeRoadMapUrl.arg(_getServerNum(x, y, 4) + 1).arg(x).arg(y).arg(zoom);
}

// 高德地图 - 混合体地图（带标注的街道图，含地名/路网）
static const QString GaoDeHybridMapUrl = QStringLiteral("https://webrd0%1.is.autonavi.com/appmaptile?lang=zh_cn&size=1&scale=1&style=7&x=%2&y=%3&z=%4");

QString GaoDeHybridMapProvider::_getURL(const int x, const int y, const int zoom, QNetworkAccessManager* networkManager) {
    Q_UNUSED(networkManager)
    return GaoDeHybridMapUrl.arg(_getServerNum(x, y, 4) + 1).arg(x).arg(y).arg(zoom);
}

// 天地图 - 卫星影像（img图层，无标注）
static const QString TiandituSatelliteMapUrl = QStringLiteral("https://t%1.tianditu.gov.cn/img_w/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=img&STYLE=default&TILEMATRIXSET=w&FORMAT=tiles&TILEMATRIX=%2&TILEROW=%3&TILECOL=%4&tk=YOUR_TIANDITU_KEY");

QString TiandituSatelliteMapProvider::_getURL(const int x, const int y, const int zoom, QNetworkAccessManager* networkManager) {
    Q_UNUSED(networkManager)
    return TiandituSatelliteMapUrl.arg(_getServerNum(x, y, 8)).arg(zoom).arg(y).arg(x);
}

// 天地图 - 标注层（cia图层，透明PNG包含地名/路网标注）
static const QString TiandituAnnotationMapUrl = QStringLiteral("http://t0.tianditu.gov.cn/img_c/wmts?tk");

QString TiandituAnnotationMapProvider::_getURL(const int x, const int y, const int zoom, QNetworkAccessManager* networkManager) {
    Q_UNUSED(networkManager)
    return TiandituAnnotationMapUrl.arg(_getServerNum(x, y, 8)).arg(zoom).arg(y).arg(x);
}
