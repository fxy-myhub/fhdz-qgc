...
Item {
    ...
    /// 基于逻辑分辨率的缩放因子，用于保证不同分辨率下组件相对窗口的比例保持一致。
    /// 以 1920x1080 为基准分辨率，按逻辑分辨率比例缩放。
    /// 注意：仅对桌面端生效，移动端保持原有逻辑。
    property real _resolutionScale: {
        if(isMobile) {
            return 1.0
        }
        // 以 1920x1080 为基准，取宽高缩放比例中较小者，避免组件超出屏幕
        var baseWidth  = 1920
        var baseHeight = 1080
        var scaleW = Screen.width  / baseWidth
        var scaleH = Screen.height / baseHeight
    ...
    */
    ...
    function _setBasePointSize(pointSize) {
        ...
        // 乘以 _resolutionScale，使组件尺寸随逻辑分辨率缩放，保证不同分辨率下组件相对窗口的比例一致
        defaultFontPixelHeight  = Math.round((_textMeasure.fontHeight * _resolutionScale)/2.0)*2
        defaultFontPixelWidth   = Math.round((_textMeasure.fontWidth * _resolutionScale)/2.0)*2
