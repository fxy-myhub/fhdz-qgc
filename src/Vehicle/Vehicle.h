...
public:
    ...
    Q_PROPERTY(uint                 customMode                  READ customMode                                                     NOTIFY flightModeChanged)
    ...
    Q_INVOKABLE void resetCounters  ();     // 重置链接状态计数器

    // Called when the message drop-down is invoked to clear current count
    Q_INVOKABLE void resetMessages();     // 重置消息计数器

    Q_INVOKABLE void virtualTabletJoystickValue(double roll, double pitch, double yaw, double thrust);

    /// Command vehicle to return to launch
    Q_INVOKABLE void guidedModeRTL(bool smartRTL);     // 命令车辆返回到起飞点

    /// Command vehicle to land at current location
    Q_INVOKABLE void guidedModeLand();     // 命令车辆降落到当前位置

    /// Command vehicle to takeoff from current location
    Q_INVOKABLE void guidedModeTakeoff(double altitudeRelative);     // 命令车辆起飞到当前位置

    /// @return The minimum takeoff altitude (relative) for guided takeoff.
    Q_INVOKABLE double minimumTakeoffAltitude();     // 获取最小起飞高度（相对高度）

    /// Command vehicle to move to specified location (altitude is included and relative)
    Q_INVOKABLE void guidedModeGotoLocation(const QGeoCoordinate& gotoCoord);     // 命令车辆移动到指定位置（包含高度，相对高度）

    /// Command vehicle to change altitude
    ///     @param altitudeChange If > 0, go up by amount specified, if < 0, go down by amount specified
    ///     @param pauseVehicle true: pause vehicle prior to altitude change
    Q_INVOKABLE void guidedModeChangeAltitude(double altitudeChange, bool pauseVehicle);        // 命令车辆改变高度

    /// Command vehicle to orbit given center point
    ///     @param centerCoord Orit around this point
    ///     @param radius Distance from vehicle to centerCoord
    ///     @param amslAltitude Desired vehicle altitude
    Q_INVOKABLE void guidedModeOrbit(const QGeoCoordinate& centerCoord, double radius, double amslAltitude);    // 命令车辆绕指定点飞行

    /// Command vehicle to keep given point as ROI
    ///     @param centerCoord ROI coordinates
    Q_INVOKABLE void guidedModeROI(const QGeoCoordinate& centerCoord);     // 命令车辆保持指定点为ROI
    Q_INVOKABLE void stopGuidedModeROI();     // 命令车辆停止保持ROI

    /// Command vehicle to pause at current location. If vehicle supports guide mode, vehicle will be left
    /// in guided mode after pause.
    Q_INVOKABLE void pauseVehicle();        // 命令车辆暂停当前位置

    /// Command vehicle to kill all motors no matter what state
    Q_INVOKABLE void emergencyStop();       // 命令车辆紧急停止

    /// Command vehicle to abort landing
    Q_INVOKABLE void abortLanding(double climbOutAltitude);

    Q_INVOKABLE void startMission();        // 命令车辆开始执行任务

    /// Alter the current mission item on the vehicle
    Q_INVOKABLE void setCurrentMissionSequence(int seq);    // 修改当前任务序列

    /// Reboot vehicle
    Q_INVOKABLE void rebootVehicle();           // 命令车辆重启

    /// Clear Messages
    Q_INVOKABLE void clearMessages();           // 清空消息

    Q_INVOKABLE void sendPlan(QString planFile);     // 发送任务计划文件
...
private:
    ...
    int     _id;                    ///< Mavlink system id 车辆的Mavlink系统ID
    ...
    bool            _receivingAttitudeQuaternion = false;    ///< 是否正在接收姿态四元数
