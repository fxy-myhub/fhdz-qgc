...
void Vehicle::_mavlinkMessageReceived(LinkInterface* link, mavlink_message_t message)
{
    ...
    // 如果链接正在运行MAVLink V2，设置最大协议版本为V2。
    unsigned mavlinkVersion = _mavlink->getCurrentVersion();    // 获取当前MAVLink版本
    ...
    // 忽略非本系统的消息 
    /*
    QGC 支持多机模式，每个 Vehicle 有独立的 _id（sysid）。
    这条消息如果不是发给我的飞控，直接丢弃。唯一的例外是 RADIO_STATUS，它来自数传电台而非飞控，但走的是同一条链路，所以放行。
    */
    ...
    // 给链接管理器先处理消息，因为它负责添加新链接并可能需要处理消息。
    // 如果链接管理器需要处理消息，它会调用 _vehicleLinkManager->mavlinkMessageReceived(link, message) 处理消息。
    ...
    case MAVLINK_MSG_ID_HOME_POSITION:     // 处理返回位置消息
        _handleHomePosition(message);
        break;
    case MAVLINK_MSG_ID_HEARTBEAT:        // 处理心跳消息
        _handleHeartbeat(message);
        break;
    case MAVLINK_MSG_ID_RADIO_STATUS:      // 处理无线电状态消息
        _handleRadioStatus(message);
        break;
    case MAVLINK_MSG_ID_RC_CHANNELS:       // 处理RC通道消息
        _handleRCChannels(message);
        break;
    case MAVLINK_MSG_ID_BATTERY_STATUS:    // 处理电池状态消息
        _handleBatteryStatus(message);
        break;
    case MAVLINK_MSG_ID_SYS_STATUS:        // 处理系统状态消息  
        _handleSysStatus(message);
        break;
    case MAVLINK_MSG_ID_RAW_IMU:                // 处理原始IMU消息
        emit mavlinkRawImu(message);
        break;
    case MAVLINK_MSG_ID_SCALED_IMU:           // 处理缩放IMU消息
        emit mavlinkScaledImu1(message);
        break;
    case MAVLINK_MSG_ID_SCALED_IMU2:            // 处理缩放IMU2消息
        emit mavlinkScaledImu2(message);
        break;
    case MAVLINK_MSG_ID_SCALED_IMU3:             // 处理缩放IMU3消息
        emit mavlinkScaledImu3(message);
        break;
    case MAVLINK_MSG_ID_EXTENDED_SYS_STATE:        /// 处理扩展系统状态消息
        _handleExtendedSysState(message);
        break;
    case MAVLINK_MSG_ID_COMMAND_ACK:           // 处理命令确认消息
        _handleCommandAck(message);
        break;
    case MAVLINK_MSG_ID_LOGGING_DATA:           // 处理日志数据消息
        _handleMavlinkLoggingData(message);
        break;
    case MAVLINK_MSG_ID_LOGGING_DATA_ACKED:         // 处理已确认日志数据消息
        _handleMavlinkLoggingDataAcked(message);
        break;
    case MAVLINK_MSG_ID_GPS_RAW_INT:            // 处理原始GPS消息  
        _handleGpsRawInt(message);
        break;
    case MAVLINK_MSG_ID_GLOBAL_POSITION_INT:    // 处理全局位置消息
        _handleGlobalPositionInt(message);
        break;
    case MAVLINK_MSG_ID_ALTITUDE:                 // 处理高度消息
        _handleAltitude(message);
        break;
    case MAVLINK_MSG_ID_VFR_HUD:                // 处理VFR HUD消息
        _handleVfrHud(message);
        break;
    case MAVLINK_MSG_ID_RANGEFINDER:            // 处理距离传感器消息
        _handleRangefinder(message);
        break;
    case MAVLINK_MSG_ID_NAV_CONTROLLER_OUTPUT:  // 处理导航控制器输出消息         
        _handleNavControllerOutput(message);
        break;
    case MAVLINK_MSG_ID_CAMERA_IMAGE_CAPTURED:  // 处理相机图像捕获消息 
        _handleCameraImageCaptured(message);
        break;
    case MAVLINK_MSG_ID_ADSB_VEHICLE:           // 处理ADSB车辆消息
        _handleADSBVehicle(message);
        break;
    case MAVLINK_MSG_ID_HIGH_LATENCY:           // 处理高延迟消息
        _handleHighLatency(message);
        break;
    case MAVLINK_MSG_ID_HIGH_LATENCY2:          // 处理高延迟2消息
        _handleHighLatency2(message);
        break;
    case MAVLINK_MSG_ID_ATTITUDE:                 // 处理姿态消息
        _handleAttitude(message);
        break;
    case MAVLINK_MSG_ID_ATTITUDE_QUATERNION:    // 处理姿态四元数消息
        _handleAttitudeQuaternion(message);
        break;
    case MAVLINK_MSG_ID_STATUSTEXT:                 // 处理状态文本消息
        _handleStatusText(message);
        break;
    case MAVLINK_MSG_ID_ORBIT_EXECUTION_STATUS: // 处理轨道执行状态消息
        _handleOrbitExecutionStatus(message);
        break;
    case MAVLINK_MSG_ID_PING:                     // 处理Ping消息
        _handlePing(link, message);
        break;
    case MAVLINK_MSG_ID_MOUNT_ORIENTATION:      // 处理挂载方向消息
        _handleGimbalOrientation(message);
        break;
    case MAVLINK_MSG_ID_OBSTACLE_DISTANCE:      // 处理障碍物距离消息
        _handleObstacleDistance(message);
        break;

    case MAVLINK_MSG_ID_EVENT:                    // 处理事件消息
    case MAVLINK_MSG_ID_CURRENT_EVENT_SEQUENCE: // 处理当前事件序列消息
    case MAVLINK_MSG_ID_RESPONSE_EVENT_ERROR:   // 处理响应事件错误消息
        _eventHandler(message.compid).handleEvents(message);
        break;

    case MAVLINK_MSG_ID_SERIAL_CONTROL:           // 处理串控制消息 
    {
    ...
    case MAVLINK_MSG_ID_CAMERA_FEEDBACK:        // 处理相机反馈消息
...
void Vehicle::_handleStatusText(mavlink_message_t& message)
{
    ...
    // 过滤掉飞控发送的 "waiting for GPS config data" 提示，避免顶部弹窗反复弹出
    if (messageText.contains(QStringLiteral("waiting for GPS config data"), Qt::CaseInsensitive)) {
        return;
    }
...
void Vehicle::_handleAttitudeWorker(double rollRadians, double pitchRadians, double yawRadians)
{
    ...
    roll = QGC::limitAngleToPMPIf(rollRadians); // 限制角度在[-PI, PI]范围内
    pitch = QGC::limitAngleToPMPIf(pitchRadians);// 限制角度在[-PI, PI]范围内
    yaw = QGC::limitAngleToPMPIf(yawRadians);    // 限制角度在[-PI, PI]范围内

    roll = qRadiansToDegrees(roll);  // 将弧度转换为角度
    pitch = qRadiansToDegrees(pitch);// 将弧度转换为角度
    yaw = qRadiansToDegrees(yaw);    // 将弧度转换为角度   
    ...
    _rollFact.setRawValue(roll);    // 设置滚转角度
    _pitchFact.setRawValue(pitch);  // 设置俯仰角度
    _headingFact.setRawValue(yaw);  // 设置航向角度
}
...
void Vehicle::_handleExtendedSysState(mavlink_message_t& message)  // 处理扩展系统状态消息
{
...
void Vehicle::_handleBatteryStatus(mavlink_message_t& message)  // 处理电池状态消息
{
